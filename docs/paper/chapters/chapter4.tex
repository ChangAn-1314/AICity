...
\subsubsection{TAM/SAM/SOM 市场空间测算}

为避免脱离项目实际，本节按智舆系统"先信阳、后河南、再区域复制"的进入路径进行三级市场测算。

\textbf{TAM (Total Addressable Market，总可服务市场)}：以中国舆情监测整体市场为口径，取2024年约110亿元、2025年约72.4亿元(不同机构统计口径不同)作为行业基准，表明赛道具备足够容量。

\textbf{SAM (Serviceable Available Market，可触达市场)}：聚焦智舆当前能力可覆盖的下沉市场客群，包括地级市政府部门、县域治理单位、本地国企园区和预算敏感型中小企业。按整体市场约15\%估算：
\[
	SAM \approx 110 \times 15\% = 16.5\text{亿元}
\]

\textbf{SOM (Serviceable Obtainable Market，可获取市场)}：按三年滚动目标测算可实际获取份额。
\[
	SOM_{3\text{年}} \approx SAM \times 0.6\% = 16.5 \times 0.6\% \approx 0.099\text{亿元}
\]
即约990万元，对应约200-300个标准化客户席位(按3-5万元/年客单价估算)，与团队"先做标杆、再做复制"的地推与渠道能力匹配。

\begin{table}[H]
	\centering
	\caption{智舆TAM/SAM/SOM测算结果}
	\begin{tabular}{L{2.8cm}L{8.2cm}L{2.2cm}}
		\toprule
		\textbf{指标} & \textbf{口径说明}                 & \textbf{规模} \\
		\midrule
		TAM         & 中国舆情监测整体市场容量(行业口径)            & 72.4-110亿元  \\
		SAM         & 下沉市场可触达客群(地级市/县域政府+国企园区+中小企业) & 16.5亿元      \\
		SOM(3年)     & 信阳试点后向河南复制，按SAM的0.6\%获取       & 约0.099亿元    \\
		\bottomrule
	\end{tabular}
\end{table}

以上测算用于验证市场进入可行性，后续将结合真实签约转化率、续费率和渠道效率按季度滚动修正。
